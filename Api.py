...
if app.isLive() == 1:
    ...
    if app.getExchange() == 'binance':
        ...
        if last_action == 'SELL' and account.getBalance(app.getQuoteCurrency()) < 5:
            raise Exception('Insufficient available funds to place buy order: ' + str(account.getBalance(
                app.getQuoteCurrency())) + ' < 50 ' + app.getQuoteCurrency() + "\nNote: A manual limit order places a hold on available funds.")
        elif last_action == 'BUY' and account.getBalance(app.getBaseCurrency()) < 0.0001:
            raise Exception('Insufficient available funds to place sell order: ' + str(account.getBalance(
                app.getBaseCurrency())) + ' < 0.1 ' + app.getBaseCurrency() + "\nNote: A manual limit order places a hold on available funds.")
...
def executeJob(sc, app=PyCryptoBot(), trading_data=pd.DataFrame()):
    ...
    if len(df_last) > 0:
        ...
        if (last_df_index != current_df_index):
            ...
            print(str(app.getTime()), '|', app.getMarket() + bullbeartext, '|', str(app.getGranularity()),
                  '| Current Price:', price)
...
try:
    ...
    if app.isTelegramEnabled():
        ...
        if app.getExchange() == 'coinbasepro':
            telegram.send(
                'Starting Coinbase Pro bot for ' + app.getMarket() + ' using granularity ' + str(app.getGranularity()))
